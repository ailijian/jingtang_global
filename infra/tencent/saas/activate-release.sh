...
if grep -Eq '^(DATABASE_(ADMIN_)?URL|DATABASE_WORKER_URL|CIAM_CLIENT_SECRET|SESSION_COOKIE_SECRET|TDMQ_AMQP_URL|YOUTUBE_OAUTH_CLIENT_SECRET|YOUTUBE_OAUTH_STATE_SECRET|FACEBOOK_APP_SECRET|FACEBOOK_OAUTH_STATE_SECRET|TENCENT_CLOUD_SECRET_(ID|KEY))=' "$saas_root/runtime.env"; then
