...
export const zhCN: Record<keyof typeof en, string> = {
  ...
  "detail.publish.instagram.queued":
    "Instagram Reel intent 已加入 provider-independent 本地候选队列。",
  "detail.publish.instagram.failed": "Instagram Reel intent 未能排队；没有执行 provider write。",
  "detail.publish.instagram.openReel": "打开 Instagram Reel",
  ...
  "detail.execution.instagram.publishing": "正在准备已确认的 Instagram Reel",
  "detail.execution.instagram.processing": "Instagram 正在处理 Reel",
  "detail.execution.instagram.published": "已发布到 Instagram Reels 标签页",
  "detail.execution.instagram.failed": "Instagram Reel 发布失败",
  "detail.execution.tiktok.publishing": "TikTok 正在安全获取视频",
  ...
  "activity.action.platform.uploaded": "平台已接受媒体传输",
  ...
  "site.external.instagramTerms": "Instagram 使用条款",
  "site.external.instagramPrivacy": "适用于 Instagram 的 Meta 隐私政策",
  "site.external.readYouTubeTerms": "阅读 YouTube 服务条款",
  "site.external.readTikTokTerms": "阅读 TikTok 服务条款",
  "site.external.readInstagramTerms": "阅读 Instagram 使用条款",
  ...
  "site.privacy.processors.body":
    "公共官网与账号受控的工作空间托管在 JINGTANG 控制的腾讯云韩国首尔资源上。官网与认证服务使用相互隔离的应用、数据库、私有对象存储、配置和日志边界。GitHub 只处理源码和合成 CI 数据。Google/YouTube、Meta/Facebook/Instagram 与 TikTok 仅在用户明确授权平台操作后，按各自的全球平台基础设施处理数据。邮件提供商处理用户发送的消息。",
  "site.privacy.youtube.title": "5. Google、YouTube、Meta、Facebook、Instagram 与 TikTok 数据",
  "site.privacy.youtube.body":
    "为账号启用相应功能后，JINGTANG 只通过官方 OAuth 使用 YouTube API Services、Meta Graph API、Instagram API with Instagram Login，或 TikTok Login Kit 与 Content Posting API，并且只执行界面明确说明的功能。Facebook 访问仅限 Meta 用户 ID/姓名、符合条件的 Page ID/名称/任务、加密用户与所选 Page Token，以及明确确认后创建的视频结果。Instagram 访问仅限 instagram_business_basic 与 instagram_business_content_publish，用于一个获授权 Professional 账号：user_id、username、加密 Token、明确确认的 MP4 与 caption、share_to_feed=false 的 REELS 容器及其返回发布状态。JINGTANG 不读取 Instagram 动态或媒体库、指标、评论、消息、广告、Tag 或无关资料。TikTok 访问仅限 user.info.basic 与 video.publish：TikTok open_id 和最小创作者显示数据、加密 Token、明确确认的 MP4、标题说明、隐私和披露选择、publish_id 与结果状态。JINGTANG 不申请 TikTok 邮箱，也不读取动态、既有视频列表、关注者、评论、消息或无关资料。确认的媒体与字段只发送给所选平台账号，平台方控制其托管副本。用户可以在 JINGTANG 内断开，并通过相应平台设置移除访问。各平台还会按自身条款和隐私政策处理数据。",
  "site.privacy.retention.title": "6. 保留与删除",
  "site.privacy.retention.body":
    "工作空间有效期间保留提供服务所需的数据。获授权删除会立即禁止访问，并在 7 天内删除适用在线数据；加密备份在 35 天内到期，恢复时会重放删除账本。无持续业务关系的网站咨询在 180 天后删除。普通 YouTube Authorized Data 与有效 Facebook、Instagram 或 TikTok 授权关联会定期验证或删除；断开、取消授权、授权失效或删除请求涉及的数据会尽快且最迟在 7 天内删除。Instagram 断开会立即停止新操作、删除 JINGTANG 保存的 Token 与账号授权数据并停止 refresh；只可保留最小化单向 callback 关联值和生命周期证据。最小化删除与安全证据最多保留 365 天。",
  ...
  "site.terms.youtube.title": "6. YouTube、Facebook、Instagram、TikTok 与外部服务",
  "site.terms.youtube.body":
    "为账号启用 YouTube、Facebook、Instagram 或 TikTok 功能后，相关使用还受对应平台条款和隐私政策约束。Facebook Page 发布只会把用户明确确认的 MP4、标题和描述发送到所选 Page。Instagram 发布只会把明确确认的 MP4 与 caption 作为 share_to_feed=false 的 Reel 发送到所选 Professional 账号。TikTok Direct Post 只会把用户明确确认的 MP4、标题说明、隐私、互动、商业内容与 AI 内容选择发送到所选 TikTok 账号；当前受控访问要求手动选择 SELF_ONLY。平台方控制处理、可见性、分发、审核和托管副本。断开 JINGTANG 不会删除该副本。其他 Integration 只有在 JINGTANG 根据已验证能力明确标注后才可使用。",
  ...
  "site.deletion.disconnect.body":
    "产品内断开会停止新的 API 操作、清除已保存 Token、停止 refresh，并清理适用 Authorized Data。JINGTANG 只在对应 Integration 已支持且已批准时执行程序化 provider revoke。对于 Instagram，JINGTANG 会立即完成本地断开，但不会声称 Meta 侧已撤销：用户必须在 Instagram 打开 Website permissions → Apps and websites → Active 并移除 JINGTANG。产品会保持“等待移除”，只有验证 deauthorization callback 后才确认 provider revocation；Token 到期不构成确认。用户也可以通过 Google 安全设置、Facebook 设置或 TikTok 应用权限控制移除访问。已签名平台数据删除请求会进入同一先禁止、后持久清理流程，并在平台要求时返回不透明状态编号。",
  "site.deletion.thirdParty.title":
    "删除 YouTube、Facebook、Instagram、TikTok 或其他平台保存的内容",
  "site.deletion.thirdParty.body":
    "删除 JINGTANG 数据或断开授权不会删除 YouTube、Facebook、Instagram、TikTok 或其他平台保存的内容。已发布 Facebook Page 视频应在 Facebook 内删除，Instagram Reel 应在 Instagram 内删除，TikTok 帖子应通过 TikTok 账号自身控制删除；其他平台托管内容请使用相应平台自身控制。JINGTANG 当前不宣称具备第三方内容删除能力。",
