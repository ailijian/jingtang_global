-- Once an Authorized Data deadline has passed, cleanup may enqueue an
-- immediately due token-key retirement child. Keep the parent retry in the
-- future so the global lifecycle queue can claim that dependency first.
