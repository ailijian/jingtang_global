...
- Design input: [`docs/deliveries/jingtang-overseas-website-saas-v1-launch/DESIGN_AUTHORITY.md`](../deliveries/jingtang-overseas-website-saas-v1-launch/DESIGN_AUTHORITY.md), Approved Revision 2
- Social review input: [`docs/deliveries/social-platform-review-enablement/BASELINE.md`](../deliveries/social-platform-review-enablement/BASELINE.md), Approved Revision 9
...
| A-12 | YouTube remains the first production integration slice. The Social Platform Developer Review Enablement Delivery may add account-controlled Facebook, TikTok and Instagram adapters only under their separately approved minimum permissions and external-review boundaries. Schedule remains disabled until a later verified capability decision. | Implemented review slices do not imply public availability; the Registry and production Gates continue to own executable public status. |
...
| A-17 | Treat the company Business Portfolio-owned Meta App ID as a durable external integration identity rather than a Review-host runtime secret. The same App ID may survive later infrastructure migration so R3 evidence is not deliberately discarded, but Review user/Page tokens, authorization rows, App Secret material, redirect/callback URLs, database state, logs and runtime configuration remain environment-bound. | This is one approved exception to A-05/A-16's separate OAuth-client rule. Before final-production cutover, revoke Review grants, destroy per-connection keys, rotate the App Secret, remove Review URLs and rerun security, policy and external-readiness checks. Continued Advanced Access or App Review approval is never inferred, and the Seoul Review runtime still cannot be promoted in place. |
| A-18 | Treat the company TikTok Organization-owned App identity and Client Key as a durable external integration identity rather than a Review-host runtime secret. The same App identity may survive later infrastructure migration so R4 evidence is not deliberately discarded, but Review Client Secret material, user tokens, authorization rows, Sandbox users, redirect URLs, database state, logs and runtime configuration remain environment-bound. | This is the second approved exception to A-05/A-16's separate OAuth-client rule. Before final-production cutover, revoke Review grants, destroy per-connection keys, rotate the Client Secret, remove Review URLs and Sandbox bindings, and rerun security, policy and external-readiness checks. Continued TikTok audit or App Review status is never inferred, and the Seoul Review runtime still cannot be promoted in place. |
| A-19 | Implement R4.5 behind a distinct Instagram Login adapter and a separate company Business Portfolio-owned Meta Business App identity. The adapter allow-lists only Instagram Login/token operations, `GET /me?fields=user_id,username`, publishing-limit lookup, one `REELS` container with fixed `share_to_feed=false`, bounded container/result reads, and one `media_publish` for the immutable intent. It never invokes the R3 Facebook Login permission-deletion route. The worker alone creates an object/method-bound, server-to-Meta signed COS read URL after final confirmation; the URL is capped at 60 minutes pending the evidence Gate and is prohibited from browser, persistence, job payload, log, audit, metric, screenshot and reviewer-media surfaces. Instagram disconnect uses the shared generation fence and local key-retirement control plane, but provider revocation is a distinct callback-confirmed state: local cleanup is immediate, refresh is prohibited, and only a verified deauthorization callback mapped through a minimized one-way correlation value may confirm Meta-side revocation. | R4.5 remains isolated from the R3 Facebook App, permissions, callbacks and subject assumptions; unknown create/publish outcomes reconcile before another write; local disconnect or token expiry cannot be presented as provider revocation. The non-secret Instagram App ID may remain durable, while Review Secret, token, account role, URL, data, logs and configuration remain environment-bound and separately gated. This is an approved build target, not evidence that the adapter, App or callback mapping exists. |
| A-20 | TikTok server-stored media uses `PULL_FROM_URL` only. After immutable final confirmation, the worker re-reads the exact private COS object and verifies MP4 type, SHA-256 and byte size, then creates a provider-only HTTPS grant on the owned application domain. The grant binds the object key, hash, size, GET method and a fixed 65-minute lifetime; the provider route supports HEAD and one byte range without redirects. COS remains private. The URL/token is secret-equivalent and cannot enter browser state, persistence, task/event payloads, audit, ordinary logs, metrics, errors, screenshots or review videos; ingress skips the exact route's access log. | This replaces only R4's historical `FILE_UPLOAD` transport. Scope, account, manual `SELF_ONLY`, immutable confirmation, single-publish and lifecycle fences remain unchanged. An ambiguous init response becomes `needs_attention` and is never automatically repeated. TikTok Dashboard URL-property verification, deployment and fresh provider evidence are separate external Gates. |
...
├── integrations/         adapter interfaces, approved YouTube/Facebook/TikTok implementations, and the separately gated Instagram target
...
| production | D3 public website remains isolated on the Tencent Cloud Lighthouse host in Seoul; SaaS/worker target a separate dedicated Tencent compute and data boundary in Seoul; each provider's production credential and authorization data is separate | Verified domains, protected deploy, least privilege, applicable provider verification/audit and truthful capability/visibility limitations |

No database, bucket, KMS key, queue, redirect URI, runtime secret, authorization data, or log destination is shared between local/test, integration, review, and production. A-17～A-19 permit only the non-secret durable Facebook Meta App ID, TikTok Organization/App identity and separate Instagram Meta App ID to survive migration; their Secret material, tokens, users/accounts, URLs, data and runtime configuration remain environment-isolated. A-16 permits review and the public website to share only the physical Lighthouse host and existing TLS ingress; their application data and deployment namespaces remain isolated.
...
For Instagram, one immutable intent owns one container and at most one successful `media_publish`. The database must persist a provider-neutral create/publish attempt state before each possible external write; success, deterministic failure and ambiguous timeout are reconciled through the exact returned or previously persisted provider identifiers. A retry may continue an existing container or result lookup but may not silently create a second container or Reel. Disconnect increments the channel generation before queued work is cancelled and local authorization/key material is retired; a stale worker must fail its fence before generating the signed media URL or issuing a provider write.
...
- Instagram callback correlation persists only a keyed one-way value sufficient to match the verified callback to one tenant-bound authorization; no token, signed request, direct provider identity, media URL or App Secret is retained in ordinary logs or audit.
...
| D-12 | Registration consent plus blocking versioned consent/re-consent before affected authorized platform API use | D2 consent record/registration; platform OAuth/API gates |
...
On 2026-08-27, the Human Owner approved the complete R4 TikTok Scope package and its company TikTok Organization-owned durable App identity. Architecture Revision 14 adds A-18 and reconciles the environment rule with that approved exception: only the non-secret App identity and Client Key may survive migration, while every Review Client Secret, user token, authorization row, Sandbox user, redirect URL, data, log and runtime setting remains isolated and must be revoked, rotated or removed before final-production cutover. The 2026-08-28 Acceptance Review repair authorization records this already-approved boundary in the current architecture authority; it does not authorize external submission, public availability or in-place promotion.

On 2026-08-28, the Human Owner approved Social Platform Review Baseline Revision 8 and then explicitly authorized its cross-authority reconciliation and R4.5 implementation planning. Architecture Revision 15 adds A-19: a separate Instagram Login App/adapter, exact two-permission and endpoint allowlists, one duplicate-safe Reels-tab-only publish, worker-only short-lived COS pull, immediate local cryptographic disconnect, manual Instagram removal and verified callback confirmation. Callback identity mapping, actual App/Standard Access configuration and COS fetch behavior remain external evidence; implementation, credentials, deployment, publishing, commit and push remain unapproved by this revision.

On 2026-08-29, the Human Owner approved Social Platform Review Baseline Revision 9 and local implementation/tests/Code Review. Architecture Revision 16 adds A-20 and replaces only TikTok's historical `FILE_UPLOAD` transport with a private-COS, provider-only `PULL_FROM_URL` boundary. The fixed 65-minute grant covers TikTok's one-hour download window plus a five-minute skew buffer and is prohibited from all browser, persistence and observability surfaces. Dashboard configuration, credentials, API calls, deployment, publishing, commit and push remain unapproved.
