...
| A-12 | YouTube remains the first production integration slice. The Social Platform Developer Review Enablement Delivery may add account-controlled Facebook and TikTok adapters only under their separately approved minimum permissions and external-review boundaries. Schedule remains disabled until a later verified capability decision. | Implemented review slices do not imply public availability; the Registry and production Gates continue to own executable public status. |
...
├── integrations/         adapter interfaces and approved YouTube/Facebook/TikTok implementations
...
No database, bucket, KMS key, queue, redirect URI, runtime secret, authorization data, or log destination is shared between local/test, integration, review, and production. A-17 and A-18 permit only the non-secret durable Meta App ID and TikTok Organization/App identity to survive migration; their Secret material, tokens, users, URLs, data and runtime configuration remain environment-isolated. A-16 permits review and the public website to share only the physical Lighthouse host and existing TLS ingress; their application data and deployment namespaces remain isolated.
