...
| A-16 | Permit one time-bounded, non-production `review` profile on the existing Seoul Lighthouse solely for Facebook/TikTok developer-review enablement. It uses `review.jingtangai.com`, isolated containers/network/volumes, a dedicated local PostgreSQL database, one direct-outbox worker, private review COS/CAM resources, a root-only local `local:v2` envelope-key boundary, separate OAuth clients and pre-created reviewer identities. It never uses production namespaces, never serves the public product CTA, and may not be promoted in place. | This is an explicit physical-co-location, direct-outbox and local-envelope-key exception to A-10/A-14 and the deployed-integration TDMQ rule, not D7 production evidence. Staging/production KMS requirements remain unchanged. Review teardown revokes credentials, destroys the local root/key store and removes review data while preserving only minimized lifecycle evidence. |
...
| review | Time-bounded isolated Docker namespace on the existing Seoul Lighthouse under A-16; dedicated local PostgreSQL, private review COS/CAM resources, root-only host-local envelope key and non-backed-up detached key store, pre-created accounts, direct-outbox worker concurrency 1, no public registration or production namespace | Platform Sandbox/Test/Review credentials only; reviewer-accessible real API slices; `noindex`; capability remains Coming Soon until separately approved and production-verified |
