...
| A-15 | Model disconnect and Workspace deletion as deny-first transactional sagas. A crash-bounded per-channel lease serializes external operations and defers revocation/deletion until work already started before the deny marker finishes; no new lease can begin after deny. The app role may append but not rewrite audit facts; a tenant-bound security-definer function may only clear actor linkage and metadata during authorized pseudonymization while immutable action facts remain protected by the audit trigger. A worker-owned clock refreshes or deletes due YouTube Authorized Data. | Provider work cannot start after a deny marker, queue completion cannot resurrect cancelled work, and a crashed lease expires safely; failed revocation remains retryable without restoring access; deletion evidence can be retained without retaining provider payload or deleted-user linkage. |
