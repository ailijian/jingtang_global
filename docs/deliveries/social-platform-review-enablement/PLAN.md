...
- Revision 9 authorization: on 2026-08-29 the Human Owner replaced only TikTok R4 server-stored-media `FILE_UPLOAD` with `PULL_FROM_URL`, preserved private COS and every Scope/account/`SELF_ONLY`/publish-count boundary, and authorized the corresponding local implementation, tests and Code Review. After the local candidate and Domain-property Gate passed, the Human Owner separately authorized a candidate branch, commit and push containing the reviewed R4.5 provider-independent implementation held disabled plus Revision 9, solely to run blocking CI and stop when CI completes. Deployment, external credentials/API calls, publishing, App Review submission and public-status changes remain unauthorized.
...
- R5: **IN PROGRESS under Human Owner authorization on 2026-08-29** for common regression/closure plus Facebook and TikTok submission-readiness work only. The secret-free execution bundle is [`R5_SUBMISSION_READINESS.md`](R5_SUBMISSION_READINESS.md). Instagram is recorded as paused and not submission-ready. External Business/Domain Verification, Advanced Access/Audit, reviewer-account provisioning, demo-video capture/upload, App Review submission, deployment, public availability, checkpoint, commit and push remain separately gated.
  ...
  - Live read-only truth checks: **PASS with an intentional production rollout hold**. All 19 authoritative website HTTPS routes rendered JINGTANG, linked to the exact Review login, exposed the deployed `2026-08-28` policy marker, included required security headers and contained no prohibited internal product wording. Review health and login returned `200` with `noindex`; platform/PostgreSQL were healthy, Worker/site were running, all four containers had zero restarts, and both backup/capacity timers were active and enabled. `pnpm site:release-check` and the candidate-version production smoke correctly failed closed because local legal version `2026-08-28-r4.5` remains pending separate production-change authorization and has not been deployed.
  - Policy revalidation: **TIKTOK LOCAL REMEDIATION AND URL PROPERTY PASS / DEPLOYMENT EVIDENCE PENDING; FACEBOOK FRESH READ PENDING**. Revision 9 resolves the local authority conflict by requiring `PULL_FROM_URL` for server-stored TikTok media, and TikTok Dashboard now reports `review.jingtangai.com` as a verified Domain property. Submission readiness still requires separately authorized deployment and fresh Human E2E evidence. Meta's official pages returned HTTP 429 to the evidence client, so the existing Facebook evidence remains authoritative locally but must be freshly inspected in the Dashboard/current official documentation immediately before submission.
...
- Revision 9 deployment/Human E2E/Acceptance: PENDING separate authorization. Historical `FILE_UPLOAD` evidence is retained as history and does not establish the new `PULL_FROM_URL` path.
