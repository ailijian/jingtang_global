...
- Local branch: `codex/r4-acceptance-prep`; `HEAD` remains `4681f98a0166d4b94ded6d980f3d2f40ac9ce1ed`, with an uncommitted local remediation that removes an unapproved TikTok endpoint and aligns current Registry/PLAN state. Draft PR #1 must not be merged without fresh authority.
...
- Current Delivery state: the second 2026-08-28 Acceptance Review returned `FAIL`. It confirmed the prior presentation/legal, CI and AC-10 remediation, but found the deployed adapter's `/v2/user/info/` call outside the approved endpoint allowlist and stale detailed Registry Gate fields. The local repair and subsequent Code Review findings are implemented, canonical verification passed, and the final Code Review rerun found no open P0/P1/P2 finding. Commit, protected CI, authorized redeployment and Acceptance re-review remain pending. Stage Acceptance remains blocked.
...
- The resulting uncommitted candidate based on `4681f98` passed the complete `pnpm verify`: formatting, lint, typecheck, builds, Terraform/release checks, 203 unit tests, contracts, 629-key bilingual catalog parity, site checks, all 29 migrations and database integration suites including the new TikTok retention regression, eight platform E2E, seven site E2E, operations/backup-restore controls and Secret scan. One moderate production dependency advisory remains below the blocking `high` threshold.
- The final Code Review rerun returned `PASS` with no open P0/P1/P2 finding. This result covers the current local working tree only until it is committed; CI and deployment must remain attributable to the resulting exact commit.

## Remaining Work, in Order

1. Commit the reviewed candidate, push the temporary branch and require protected CI to pass the exact commit.
2. Deploy that immutable commit to Review under the current Human Owner instruction, reinstall the existing maintenance timers and complete independent health/noindex/release-pointer smoke. No reconnect or additional TikTok publish is required for this endpoint removal.
3. Stop when the deployed commit, protected CI and every prerequisite Gate make R4 ready for `acceptance-review`. Keep TikTok `coming_soon`, every public capability `not_available`, and `production_available: false`; do not invoke the review until the Human Owner explicitly instructs it.
...
```text
Continue the R4 endpoint-allowlist remediation from docs/deliveries/social-platform-review-enablement/R4_HANDOFF.md. Read AGENTS.md, docs/README.md, the approved BASELINE.md, current PLAN.md, Architecture, Security/Data, public-site config and config/integrations.yaml first. The locally verified endpoint-removal and Registry repair passed the final Code Review with no open P0/P1/P2 finding. Under the current authorization, commit the reviewed candidate, obtain protected CI for that exact commit, deploy it to Review, reinstall the maintenance timers and run independent smoke checks. Stop when the Delivery is ready for Acceptance Review; do not invoke $acceptance-review without the Human Owner's next explicit instruction. Do not reconnect TikTok, publish another video, merge, checkpoint, submit TikTok review, configure Production or enable public availability without separate authority.
```
