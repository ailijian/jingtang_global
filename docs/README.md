...
| SaaS/worker change control, monitoring, backup/restore, incident response, vulnerability management, and production-access procedures | [Operations](OPERATIONS.md); derived operator guide: [current Seoul manual deployment and maintenance](../infra/tencent/MANUAL_DEPLOYMENT.md) | `current_state` operating procedure | Operations is repository-wide authority; the derived guide provides executable current-host mechanics. Neither repository checks nor the guide substitute for D7 Tencent Cloud production evidence. |
